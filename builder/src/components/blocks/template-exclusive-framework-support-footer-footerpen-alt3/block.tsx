...
import {
  buildColumn,
  renderFrameworkStructuredFooter,
} from "@/components/blocks/_shared/framework-structured-footer";
import {
  ArrowLeft,
  ArrowRight,
  ChevronLeft,
...
export default function TemplateExclusiveFrameworkSupportFooterFooterpenAlt3({ id, clzbctext, pcfsktext, pcfskhref, g6yuftext, rdotctext, rdotchref, hu3uvtext, nqugstext, nqugshref, om4ndtext, tn1zntext, tn1znhref, d8wcftext, wwiyjtext, wwiyjhref, zao7qtext, j01f7href, fonnptext, irp3atext, quashtext, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, clzbctext, pcfsktext, pcfskhref, g6yuftext, rdotctext, rdotchref, hu3uvtext, nqugstext, nqugshref, om4ndtext, tn1zntext, tn1znhref, d8wcftext, wwiyjtext, wwiyjhref, zao7qtext, j01f7href, fonnptext, irp3atext, quashtext });
  ...
  return renderFrameworkStructuredFooter({
    sectionId: merged.id || DEFAULT_PROPS.id,
    sectionClassName,
    sectionStyle: mergedSectionStyle,
    revealRef: sectionMotion?.level === "off" ? undefined : reveal.ref,
    columns: [
      buildColumn(clzbctext, pcfsktext, pcfskhref),
      buildColumn(g6yuftext, rdotctext, rdotchref),
      buildColumn(hu3uvtext, nqugstext, nqugshref),
      buildColumn(om4ndtext, tn1zntext, tn1znhref),
    ],
    subscribeTitle: d8wcftext,
    subscribeBody: wwiyjtext,
    subscribeBodyHref: wwiyjhref,
    subscribeInputPlaceholder: zao7qtext,
    subscribeInputHref: j01f7href,
    subscribeButtonLabel: fonnptext,
    legal: irp3atext,
    currency: quashtext,
    theme: merged?.theme,
  });

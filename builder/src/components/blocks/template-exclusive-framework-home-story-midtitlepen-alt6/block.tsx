...
export default function TemplateExclusiveFrameworkHomeStoryMidtitlepenAlt6({ id, mid1text, fixlinetext, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, mid1text, fixlinetext });

...
export default function TemplateExclusivePenSiteJabilSolutionStorySourceplanpenAlt7({ id, sourceimgimagesrc, peytext, wwkketext, psubtext, pbtnhref, pbtnttext, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, sourceimgimagesrc, peytext, wwkketext, psubtext, pbtnhref, pbtnttext });

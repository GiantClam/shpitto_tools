...
export default function TemplateExclusivePenSiteOerlikonAboutStoryIntropenAlt2({ id, breadcrumbatext, introtextatext, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, breadcrumbatext, introtextatext });

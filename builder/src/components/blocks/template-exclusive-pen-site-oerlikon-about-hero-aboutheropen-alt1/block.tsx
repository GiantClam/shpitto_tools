...
export default function TemplateExclusivePenSiteOerlikonAboutHeroAboutheropenAlt1({ id, herobgaimagesrc, herotitleatext, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, herobgaimagesrc, herotitleatext });

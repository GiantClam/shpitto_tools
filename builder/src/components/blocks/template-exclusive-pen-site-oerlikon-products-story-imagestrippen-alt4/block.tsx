...
export default function TemplateExclusivePenSiteOerlikonProductsStoryImagestrippenAlt4({ id, strip1imagesrc, label1text, strip2imagesrc, label2text, strip3imagesrc, label3text, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, strip1imagesrc, label1text, strip2imagesrc, label2text, strip3imagesrc, label3text });

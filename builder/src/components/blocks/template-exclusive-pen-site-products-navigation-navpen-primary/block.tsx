...
const SECTION_TREE = {
  ...
  "children": [
    ...
    },
    {
      ...
      "children": [],
      "textProp": "centernavtext",
      "hrefProp": "centernavhref"
    },
    {
      "type": "text",
      "id": "iR903",
      ...
      "children": [],
      "textProp": "rightnavtext",
      "hrefProp": "rightnavhref"
    }
  ]
};
const DEFAULT_PROPS = {
...
const NAV_ACTIVE_COLOR = "#111111";
const NAV_INACTIVE_COLOR = "#4A4A4A";
...
const splitNavItems = (value = "") =>
  String(value || "")
    .split(/\s{2,}|\n+/)
    .map((item) => item.trim())
    .filter(Boolean);

const resolveFrameworkNavHref = (label = "", fallback = "/") => {
  const token = String(label || "").trim().toLowerCase();
  if (!token) return fallback;
  if (token === "products" || token === "laptops" || token === "desktops" || token === "marketplace" || token === "cart") {
    return "/products";
  }
  if (token === "support") return "/support";
  if (token === "sign in") return "/contact";
  return fallback;
};

const renderNavGroup = ({ items = [], baseStyle = {} as React.CSSProperties, fallbackHref = "/", gap = 28, currentPathToken = "/" }) =>
  React.createElement(
    "div",
    {
      style: {
    ...
    ...items.map((item) => {
      const href = resolveFrameworkNavHref(item, fallbackHref);
      const hrefPathToken = normalizeNavPath(href);
      const isActive = Boolean(hrefPathToken) && hrefPathToken === currentPathToken;
      return React.createElement(
        "a",
        {
          ...
          style: {
            ...baseStyle,
            textDecoration: "none",
            color: isActive ? NAV_ACTIVE_COLOR : baseStyle.color || "inherit",
            fontWeight: isActive ? "600" : baseStyle.fontWeight,
          },
        },
        item
      );
...
export default function TemplateExclusivePenSiteProductsNavigationNavpenPrimary({ id, logotext, logohref, centernavtext, centernavhref, rightnavtext, rightnavhref, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, logotext, logohref, centernavtext, centernavhref, rightnavtext, rightnavhref });
  ...
  const rootStyle = { ...(SECTION_TREE.style || {}) } as React.CSSProperties;
  const logoStyle = { ...((SECTION_TREE.children?.[0] || {}).style || {}) } as React.CSSProperties;
  const centerStyle = { ...((SECTION_TREE.children?.[1] || {}).style || {}) } as React.CSSProperties;
  const rightStyle = { ...((SECTION_TREE.children?.[2] || {}).style || {}) } as React.CSSProperties;
  delete centerStyle.wordSpacing;
  delete rightStyle.wordSpacing;
  centerStyle.whiteSpace = "nowrap";
  rightStyle.whiteSpace = "nowrap";
  return (
    <section
    ...
    >
      <div style={rootStyle}>
        <a href={merged.logohref || "/"} style={{ ...logoStyle, textDecoration: "none" }}>
          {merged.logotext}
        </a>
        {renderNavGroup({
          items: splitNavItems(merged.centernavtext),
          baseStyle: centerStyle,
          fallbackHref: merged.centernavhref || "/products",
          gap: 40,
          currentPathToken,
        })}
        {renderNavGroup({
          items: splitNavItems(merged.rightnavtext),
          baseStyle: rightStyle,
          fallbackHref: merged.rightnavhref || "/support",
          gap: 28,
          currentPathToken,
        })}
      </div>

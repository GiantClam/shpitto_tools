...
export default function TemplateExclusivePenSiteAboutHeroAboutheropenAlt3({ id, herobannerimagesrc, herotagtexttext, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, herobannerimagesrc, herotagtexttext });

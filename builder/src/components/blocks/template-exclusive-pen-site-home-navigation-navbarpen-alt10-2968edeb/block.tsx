...
export default function TemplateExclusivePenSiteHomeNavigationNavbarpenAlt10({ id, brandLogoHomeimagesrc, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, brandLogoHomeimagesrc });

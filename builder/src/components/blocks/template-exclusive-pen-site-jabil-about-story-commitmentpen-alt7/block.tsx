...
export default function TemplateExclusivePenSiteJabilAboutStoryCommitmentpenAlt7({ id, historyeyebrowtext, historytitletext, historylinktxttext, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, historyeyebrowtext, historytitletext, historylinktxttext });

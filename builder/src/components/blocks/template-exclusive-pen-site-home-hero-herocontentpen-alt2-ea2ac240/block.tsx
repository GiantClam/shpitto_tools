...
const DEFAULT_PROPS = {
  "id": "y8Rr8",
  "herokickertext": "MASTER & DYNAMIC / SIGNATURE",
  "heroheadlinetext": "Over-Ear Sound Tuned For Long Sessions And Deep Focus",
  "herobodytext": "A warmer, studio-led listening profile with premium materials and immersive wireless performance.",
...
const buildNodeStyle = (node, merged, sectionMotion, sectionKindToken, keyPath, currentPathToken = "/") => {
  const style = { ...(node?.style || {}) };
  const rawHref = node?.hrefProp ? String(merged?.[node.hrefProp] || "").trim() : "";
  if (keyPath === "root") {
    const numericRootWidth = resolveNumericDimension(style?.width);
    if (numericRootWidth > 0) {
...
const renderNode = (node, merged, sectionMotion, sectionKindToken, key = "root", ancestorHasLink = false, currentPathToken = "/") => {
  if (!node || typeof node !== "object") return null;
  const style = buildNodeStyle(node, merged, sectionMotion, sectionKindToken, key, currentPathToken);
...
export default function TemplateExclusivePenSiteHomeHeroHerocontentpenAlt2({ id, herokickertext, heroheadlinetext, herobodytext, heroctatexttext, ...rest }) {
  const pathname = usePathname();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, herokickertext, heroheadlinetext, herobodytext, heroctatexttext });
  assignDefined(merged, rest);
  const currentPathToken = normalizeNavPath(merged?.currentPath || pathname || "/");

...
export default function TemplateExclusivePenSiteOerlikonRDSocialproofQuotesectionpenAlt3({ id, f4y7stext, wdiuetext, portraitaimagesrc, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, f4y7stext, wdiuetext, portraitaimagesrc });

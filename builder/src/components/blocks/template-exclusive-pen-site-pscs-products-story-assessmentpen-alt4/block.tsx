...
export default function TemplateExclusivePenSitePscsProductsStoryAssessmentpenAlt4({ id, assesstitletext, assessbodytext, list1text, assessimgimagesrc, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, assesstitletext, assessbodytext, list1text, assessimgimagesrc });

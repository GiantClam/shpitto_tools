...
export default function TemplateExclusivePenSiteJabilCapabilitiesStoryCapintropenAlt2({ id, introtexttext, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, introtexttext });

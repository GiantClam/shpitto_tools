...
export default function TemplateExclusivePenSitePscsSolutionSocialproofTestimonialpenAlt6({ id, testtitletext, testbodytext, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, testtitletext, testbodytext });

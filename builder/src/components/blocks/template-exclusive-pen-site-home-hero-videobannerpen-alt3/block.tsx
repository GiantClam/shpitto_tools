...
export default function TemplateExclusivePenSiteHomeHeroVideobannerpenAlt3({ id, vbgimagesrc, videotitletext, playtritext, videologotext, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, vbgimagesrc, videotitletext, playtritext, videologotext });

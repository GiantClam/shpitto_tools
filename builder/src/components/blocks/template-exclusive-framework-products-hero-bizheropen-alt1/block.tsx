...
export default function TemplateExclusiveFrameworkProductsHeroBizheropenAlt1({ id, herot1text, herosubtext, herorimagesrc, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, herot1text, herosubtext, herorimagesrc });

...
export default function TemplateExclusivePenSiteOerlikonRDStoryResearchdevelopmentpenAlt7({ id, cltputext, txcv3text, rdbtnahref, bgnk8text, rdimgaimagesrc, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, cltputext, txcv3text, rdbtnahref, bgnk8text, rdimgaimagesrc });

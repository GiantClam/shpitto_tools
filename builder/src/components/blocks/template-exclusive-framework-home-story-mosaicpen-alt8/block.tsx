...
export default function TemplateExclusiveFrameworkHomeStoryMosaicpenAlt8({ id, mosaicimagesrc, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, mosaicimagesrc });

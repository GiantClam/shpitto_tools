  ...
  "children": [
    {
      ...
      "style": {
        ...
        "width": 756,
        "height": 508,
        "background": "linear-gradient(160deg, #0F274499 0%, #0A0E14AA 45%, #0A0E14F2 100%)",
        "position": "absolute",
        "left": 340,
        "top": 0
      },
...
export default function TemplateExclusivePenSiteUrsaAboutStoryAboutcompanypenAlt3({ id, companytitletext, companybodytext, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, companytitletext, companybodytext });

...
export default function TemplateExclusivePenSiteOerlikonRDStoryBreadcrumbpenAlt2({ id, fz8zltext, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, fz8zltext });

...
const parseBulletOptions = (value = "") =>
  String(value || "")
    .split(/\n+/)
    .map((entry) => entry.replace(/^[•\-\s]+/, "").trim())
    .filter(Boolean);

const renderBusinessTeamForm = (node, merged, key = "connect-team-form") => {
  ...
  const stackedField = (label: string, control: React.ReactNode) => (
  ...
  return (
    ...
    >
      <div style={{ flex: "0 1 320px", display: "grid", gap: 14 }}>
        <div
          style={{
            color: "#1F2737",
            fontFamily: "Inter",
      ...
      <form
        ...
        style={{ flex: "1 1 560px", display: "grid", gap: 14, minWidth: 0 }}
      >
        <div
          style={{
            display: "grid",
        ...
        >
          {stackedField(merged.fylwytext, <input type="text" name="firstName" placeholder={merged.fylwytext} style={inputStyle} />)}
          {stackedField(merged.vazprtext, <input type="text" name="lastName" placeholder={merged.vazprtext} style={inputStyle} />)}
        </div>
        {stackedField(merged.zbknttext, <input type="text" name="company" placeholder={merged.zbknttext} style={inputStyle} />)}
        {stackedField(merged.w2r4vtext, <input type="email" name="workEmail" placeholder="name@company.com" style={inputStyle} />)}
        <div
          style={{
            display: "grid",
            gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))",
        ...
        >
          {stackedField(merged.hsfwotext, <input type="text" name="country" placeholder={merged.hsfwotext} style={inputStyle} />)}
          {stackedField(
            merged.dimyjtext,
            <select name="programType" defaultValue="" style={inputStyle}>
              <option value="" disabled>
                Select a program
              </option>
              <option value="procurement">Procurement</option>
              <option value="fleet-refresh">Fleet refresh</option>
        ...
        {stackedField(
          merged.check1text,
          <select name="quantity" defaultValue="" style={inputStyle}>
            <option value="" disabled>
              Select quantity
            </option>
        ...
        {stackedField(merged.industrytext, <input type="text" name="industry" placeholder={merged.industrytext} style={inputStyle} />)}
        {stackedField(merged.notestext, <textarea name="notes" placeholder="Tell us about your deployment needs" style={textareaStyle} />)}
        <label
          style={{
            display: "flex",
            alignItems: "flex-start",
...
const renderNode = (
  ...
  if (String(node?.name || "").trim() === "ConnectTeam") {
    return renderBusinessTeamForm(node, merged, key);
  }

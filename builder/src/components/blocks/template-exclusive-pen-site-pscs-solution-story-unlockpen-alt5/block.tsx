...
export default function TemplateExclusivePenSitePscsSolutionStoryUnlockpenAlt5({ id, unlocktitletext, unlockbodytext, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, unlocktitletext, unlockbodytext });

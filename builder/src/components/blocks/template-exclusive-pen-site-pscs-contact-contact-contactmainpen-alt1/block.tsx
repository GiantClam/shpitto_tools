...
export default function TemplateExclusivePenSitePscsContactContactContactmainpenAlt1({ id, contacttitletext, label1text, label2text, label3text, label4text, label5text, label6text, label7text, submittexttext, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, contacttitletext, label1text, label2text, label3text, label4text, label5text, label6text, label7text, submittexttext });

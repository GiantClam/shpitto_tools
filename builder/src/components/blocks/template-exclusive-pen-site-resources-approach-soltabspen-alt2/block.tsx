...
export default function TemplateExclusivePenSiteResourcesApproachSoltabspenAlt2({ id, tab1ttext, tab2text, tab3text, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, tab1ttext, tab2text, tab3text });

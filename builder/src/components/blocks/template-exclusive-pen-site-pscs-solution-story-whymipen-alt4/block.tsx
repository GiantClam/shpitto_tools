...
export default function TemplateExclusivePenSitePscsSolutionStoryWhymipenAlt4({ id, whytitletext, whybodytext, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, whytitletext, whybodytext });

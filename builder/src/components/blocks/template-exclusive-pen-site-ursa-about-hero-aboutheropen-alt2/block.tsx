  ...
  "children": [
    {
      ...
      "style": {
        ...
        "width": 1440,
        "height": 432,
        "background": "linear-gradient(90deg, #0A0F18D9 0%, #0A0F1840 60%, #0A0F18CC 100%)",
        "position": "absolute",
        "left": 0,
        "top": 0
      },
...
export default function TemplateExclusivePenSiteUrsaAboutHeroAboutheropenAlt2({ id, aboutHeroimagesrc, herotitletext, herodesctext, herodesctext2, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, aboutHeroimagesrc, herotitletext, herodesctext, herodesctext2 });

...
export default function TemplateExclusivePenSitePscsBlogHeroBlogheropenAlt1({ id, heroimgimagesrc, blogtitletext, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, heroimgimagesrc, blogtitletext });

...
export default function TemplateExclusivePenSiteSandvikCustomerServiceStorySvcpenAlt1({ id, svc1text, svc2text, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, svc1text, svc2text });

  ...
  "children": [
    {
      ...
      "style": {
        ...
        "width": 1440,
        "height": 580,
        "background": "linear-gradient(90deg, #0A0F18E4 0%, #0A0F1822 65%, #0A0F18B4 100%)",
        "position": "absolute",
        "left": 0,
        "top": 0
      },
...
export default function TemplateExclusivePenSiteUrsaSolutionHeroHeropenAlt1({ id, heroimagesrc, h1text, h2text, hdesctext, herodesctext, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, heroimagesrc, h1text, h2text, hdesctext, herodesctext });

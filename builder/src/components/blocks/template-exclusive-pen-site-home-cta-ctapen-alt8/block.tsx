...
export default function TemplateExclusivePenSiteHomeCtaCtapenAlt8({ id, ctahref, ctaimgimagesrc, xkitztext, vrdiwtext, ctabtnhref, ...rest }) {
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const providerMotionMode = useMotionMode();
  const merged = assignDefined({ ...DEFAULT_PROPS }, { id, ctahref, ctaimgimagesrc, xkitztext, vrdiwtext, ctabtnhref });
